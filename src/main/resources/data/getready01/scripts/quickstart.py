...
def _apply_metrics_workaround() -> None:
    """Work around cosmos_xenna metrics collection failing when GPU/cgroup stats are None (e.g. CPU-only nodes)."""
    try:
        from cosmos_xenna.pipelines.private import monitoring as _monitoring

        _original_update = _monitoring.RayResourceMonitor.update

        def _patched_update(self: _monitoring.RayResourceMonitor) -> dict:
            try:
                return _original_update(self)
            except Exception as e:  # noqa: BLE001
                logger.warning("Metrics collection failed, using empty node data: %s", e)
                return {}

        _monitoring.RayResourceMonitor.update = _patched_update
    except ImportError:
        pass
