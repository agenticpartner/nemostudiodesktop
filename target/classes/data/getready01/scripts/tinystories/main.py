...
def main(args: argparse.Namespace) -> None:
    ...
    # Run the pipeline. RayActorPoolExecutor is used to avoid GPU monitoring issues
    # (e.g. cosmos_xenna fails when GPU memory is not reported, as with some NVIDIA GB10 setups).
    results = pipeline.run(executor=RayActorPoolExecutor())
    end_time = time.time()
    ...
    if args.keep_ray_running:
        # Unregister so atexit won't call ray_client.stop() when the script exits
        atexit.unregister(ray_client.stop)
        print(
            "\nRay cluster is still running (dashboard stays available). "
            "To stop it later, run: ray stop"
        )
    else:
        ray_client.stop()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--keep_ray_running",
        action="store_true",
        help="Do not stop the Ray cluster when the pipeline finishes (keeps the dashboard at http://localhost:8265/ available; stop later with: ray stop)",
    )
